...
var throttle = function throttle(fn) {
  ...
  return function () {
    ...
    if (!running) {
      running = true; // TODO: Use timeout if RAF is unavailable
...
function (_React$PureComponent) {
  ...
  function InfiniteScroll() {
    ...
    _this.onScroll = throttle(function () {
      ...
      var thresholdPoint = scrollHeight - clientHeight - clientHeight * threshold;
      var thresholdReachable = clientHeight < thresholdPoint;
      var reachedTreshold = scrollTop >= thresholdPoint;
      ...
      _this._lastScrollTop = scrollTop;
      var reachedEnd = scrollTop + clientHeight === scrollHeight; // Reaching the treshold can occur multiple times, handle differently
  ...
  _createClass(InfiniteScroll, [{
    ...
    value: function componentDidMount() {
      ...
      node.scrollTo && node.scrollTo({
        top: 0
      }); // Dunno why, but for some reason the node is scrolled to bottom when mounting
    ...
    value: function render() {
      return React.createElement("div", {
        style: {
          maxHeight: this.props.height || 'calc(100%)',
          overflowY: 'auto'
        },
        ref: this._ref,
