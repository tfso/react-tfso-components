  ...
  try {
    for (var _iterator = sortedItemLayouts[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      var _loop = function _loop() {
        var test = _objectSpread({}, layout, {
          row: row
        });

        if (!Object.values(newLayout).find(function (otherItem) {
      ...
      newLayout[_key] = itemWasChanged ? _objectSpread({}, items[_key], _defineProperty({}, screenType, layout)) : items[_key]; // don't want to duplicate items.
    }
  } catch (err) {
